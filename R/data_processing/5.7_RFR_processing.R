...
raw_path = './data_raw/20260105_fig5b_raw/'
out_path = './data/heliospectra_measurements/fig5b/'

regime_dir = "data/regimes/fig5b_R_FR_demo/5b_RFR_intensities.csv"
peaks_dir = 'data/heliospectra_measurements/calibration/Apollo_Calib_20240827/Apollo_calibration_medianPeaks_20240827.Rda'
target_dir = './data/algorithm_testing/fig5b_R_FR_demo/5b_targets.csv'
...
fn = paste0(out_path, '5b_raw_', date_measured, '.Rda')
save(fig6_raw, file=fn)
rm(fig6_raw)

# 2. Trimming ====

message("5.7.2 Trimming")
...
message("5.7.3. Annotate with useful columns")
...
message("5.7.4 Calculate R:FR ratio")
...
fig5b_RFR_ratio$measured_RFR = fig5b_RFR_ratio$measured_R / fig5b_RFR_ratio$measured_FR

# 5. Make measurements df ====

message("5.7.5 Make measurements df")
...
message("5.7.6 Format & export")

## Arrange columns sensibly

fig5b_spectrum = measurements |> select(filename, integration_time, scans, 
                                   time, event, middle_time,
                                   wavelength, peak,
                                   irradiance, watts, mol, umol)

fig5b_RFR_ratio = fig5b_RFR_ratio |> select(event, solar_elevation_angle, time_approx, measured_time,
                                          filename,
...
fn = paste0(out_path, 'fig5b_data_', date_measured, '.Rda')
save(fig5b_spectrum, fig5b_RFR_ratio, measurements_summary, file=fn)